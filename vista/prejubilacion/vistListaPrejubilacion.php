...
<section class="seccion">
    ...
    $query2 = 'SELECT COUNT(*) FROM prejubilaciones';
    $res = CConecta::$db->query($query2);
    if($res){
        $prejubilaciones = $res->fetch_assoc()['COUNT(*)'];
        if(!empty($prejubilaciones)){
            ?>
    ...
    if(empty($arregloPaginado)){
        if(isset($_POST['datoBuscar']) && isset($nomBuscar)){
            ...
            ?> <h3 class="titulo">Ning&uacute;n Docente cumple con los requisitos para el proceso Prejubilatorio</h3><?php
        }
    }else{
        ?>
