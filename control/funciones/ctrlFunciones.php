...
function getRango($id_rango){
    $query = 'SELECT nombre FROM rangos WHERE id_rango='.$id_rango.'';
    $res = CConecta::$db->query($query);    
    if($res){
        $estado = $res->fetch_assoc()['nombre'];
    }
    return $estado;
}
function getEstado($id_estado){
    $query = 'SELECT estado FROM estados WHERE id_estado='.$id_estado.'';
    $res = CConecta::$db->query($query);    
    if($res){
...
function getMunicipio($id_municipio){
    $query = 'SELECT municipio FROM municipios WHERE id_municipio='.$id_municipio.'';
    $res = CConecta::$db->query($query);    
    if($res){

    }
    $municipio = $res->fetch_assoc()['municipio'];
    return $municipio;
}
function getLocalidad($id_localidad){
    $query = 'SELECT localidad FROM localidades WHERE id_localidad='.$id_localidad.'';
    $res = CConecta::$db->query($query);    
    if($res){
        $localidad = $res->fetch_assoc()['localidad'];
    }
    return $localidad;
}
function getGradoEstudios($id_grado_estudio){
    $query = 'SELECT grado_estudios FROM grados_estudios WHERE id_grado_estudio='.$id_grado_estudio.'';
    $res = CConecta::$db->query($query);    
    if($res){
        $grado = $res->fetch_assoc()['grado_estudios'];
    }
    return $grado;
}
